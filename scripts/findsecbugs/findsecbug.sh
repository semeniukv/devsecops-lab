...
python3 ${WORKSPACE}/scripts/vuln_check_rule.py --scanner=findsecbug --scan_id=$SCAN_ID --username=${ARCHERY_USER} --password=${ARCHERY_PASS} --host=${ARCHERYSEC_HOST} --high=${FSBHIGH} --medium=${FSBMEDIUM}

job_status=`python3 ${WORKSPACE}/scripts/vuln_check_rule.py --scanner=findsecbug --scan_id=$SCAN_ID --username=${ARCHERY_USER} --password=${ARCHERY_PASS} --host=${ARCHERYSEC_HOST} --high=${FSBHIGH} --medium=${FSBMEDIUM} | grep SUCCESS`
