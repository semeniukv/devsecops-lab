...
if args.scanner == 'arachni_scan':
    time.sleep(5)
    archery = archery_host()
    web_scan_result = archery.arachni_scan_status(
        auth=archery_auth(),
        scan_id=args.scan_id,
    )
    results = web_scan_result.data_json()
...
if args.scanner == 'dependencycheck':
    time.sleep(5)
    archery = archery_host()
    web_scan_result = archery.dependencycheck_scan_status(
        auth=archery_auth(),
        scan_id=args.scan_id,
    )
    results = web_scan_result.data_json()
